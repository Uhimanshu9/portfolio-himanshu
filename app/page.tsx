...
const roles = ["AI Engineer", "Agent Builder", "Systems Researcher", "Open Source Builder"];
...
const notes = [
  { title: "DoW attack mitigation with TEEs", type: "Research", detail: "IEEE IC3I 2025 accepted", meta: "100% mitigation · 12.9% latency overhead" },
  { title: "LOVA — LiveOps validation platform", type: "Scopely", detail: "Multi-agent systems", meta: "50K+ production fields · 30-minute QA cycle" },
  { title: "Autonomous vehicle health analysis", type: "Transvolt Mobility", detail: "Data science internship", meta: "10 trucks flagged · INR 8L saved per battery" },
  { title: "Competitive systems building", type: "Achievements", detail: "CodeCrafters · CodeChef · FishHunt CTF", meta: "Global Rank 141 · Rank 52 · Runner-up" },
];
...
function Navbar({ activeSection }: { activeSection: string }) {
  const links = [{ label: "Home", href: "#home", id: "home" }, { label: "Work", href: "#work", id: "work" }, { label: "Resume", href: "#resume", id: "resume" }];
...
function NoteRow({ note }: { note: (typeof notes)[number] }) {
  return <a href="#resume" className="journal-row group flex items-center gap-4 rounded-[30px] border border-stroke bg-surface/30 p-3 sm:gap-6 sm:rounded-full sm:p-3"><div className="note-index flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-stroke font-display text-2xl italic text-text-primary sm:h-16 sm:w-16">↗</div><div className="min-w-0 flex-1"><div className="mb-1 text-[9px] uppercase tracking-[.22em] text-muted">{note.type}</div><h3 className="truncate text-sm text-text-primary sm:text-base">{note.title}</h3></div><div className="hidden max-w-sm text-right md:block"><div className="text-xs text-text-primary/80">{note.detail}</div><div className="mt-1 text-[10px] uppercase tracking-[.15em] text-muted">{note.meta}</div></div><span className="pr-2 text-base text-text-primary transition-transform group-hover:translate-x-1">↗</span></a>;
}
